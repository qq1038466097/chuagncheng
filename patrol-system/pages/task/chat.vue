<template>
	<view class='radarBg chatBg'>
		<view class='chat-group'>
			<text class='title'>附近成员</text>
			<view class="chat-ul">
				<view class='chat-li' v-for="(man,i) in manList" v-if="i<5"  @click="toChat(man.name,man.memberId)">
					<view class='img-box'>
						<image :src='setHead(man.avatar)' class='img'></image>
					</view>
					<text class='name'>{{man.name}}</text>
				</view>
				<view class='chat-li' @click="togroup">
					<view class='img-box img-gray'>
						<image src='/static/images/icon-mask.png' class='img'></image>
					</view>
					<text class='name'>({{manList.length}})</text>
				</view>
			</view>
		</view>
...
<script>
    ...
    import {chatMan,creatChat,chatList,sendMs,imMsgList,imManList,msRead} from '@/api/user.js'
	...
	export default {
		...
		onLoad(options){
			...
			imManList(_ars2).then(res=>{
				if(res!==null){
					...
					for(let i=0; i<res.length; i++){
						if(res[i].memberId == _that.employee.id){
							_that.icrmId = res[i].icrmId
							_icrmId=res[i].icrmId
							break;
						}
					}
					if(options.numbers>0){
						...
						let _a={}
						_a.icrmId = _icrmId
						_a.lastReadTime = _that.readTime
						_that.setRead(_a)
					}
			...
			//获取消息
			let _ars={}
			_ars.chatId = options.chatId
			imMsgList(_ars).then(res=>{
				if(res!==null){
					let _firTime = res[0].createTime
					let _res = res.reverse()
					_that.messageList = _res
					//最新一条数据，时间
					_that.newTime = res[0].createTime
				}
			}).catch(err=>{})
		},
        onUnload(){
        ...
        　　this.contentViewHeight = res.windowHeight - uni.getSystemInfoSync().screenWidth / 750 * (100) - 70 - 100; 
            // 开启ws
		...
		methods: {
			...
			toChat(name,id){
				uni.navigateTo({
					url: '/pages/task/chat-one?names='+name+'&ids='+id+'&employeeId='+this.employeeId,
				});
			},
			//群组
			togroup(){
