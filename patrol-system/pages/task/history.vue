<template>
	<view class="content dotBg">
		...
		<view class='history-top'>
            <text class='time'>{{historydetail.checkTime}}</text>
            <text class='title green' v-if='parseInt(historydetail.checkResult)==1'>优秀</text>
            <text class='title green' v-if='parseInt(historydetail.checkResult)==2'>合格</text>
			<text class='title red' v-if='parseInt(historydetail.checkResult)==3&&details.watchStatus==2&&details.watchResult==3&&details.fixStatus==2'>不合格·待处理</text>
			<text class='title red' v-if='parseInt(historydetail.checkResult)==3&&details.fixStatus==3'>不合格·处理中</text>
			<text class='title red' v-if='parseInt(historydetail.checkResult)==3&&details.fixStatus==4'>不合格·已处理</text>
            <text class='time'>
                <text v-if='historydetail.goodCount!==0'>{{historydetail.goodCount}}个优秀、</text>
                <text v-if='historydetail.passCount!==0'>{{historydetail.passCount}}个合格、</text>
                <text v-if='historydetail.faultCount!==0'>{{historydetail.faultCount}}个不合格</text>
            </text>
            <view class='shows'>
                <text>巡查员：{{names}} </text>
                <text class='phone' @click.stop="callPhone(phones)">{{setPhones(phones)}}</text>
            </view>
        </view>
		<!--展开更多-->
		<view class='history-data' v-if="ismore==true&&checkRecordFlows.length>0" @click="setMore(false)">
			<view class='content'>
				<view class='content-li' v-for="(item,i) in checkRecordFlows" :key='i'>
					<view :class='item.opType + " bg"'></view>
					<view class='showBox'>
						<text class='title'>{{item.flowDesc}}</text>
						<text class='time'>{{item.opTime}}</text>
					</view>
					<view class='showBox' v-if='item.opType=="pushGrid"'>
						<text class='title'>工单系统 - 已派单</text>
					</view>
					<view class='showBox' v-if='item.opType!=="pushGrid"'>
						<text class='title'>{{item.employeeName}}</text>
						<text class='phone'>{{setPhones(item.employeePhone)}}</text>
					</view>
				</view>
			</view>
			<view class='right-icon'>
				<image class='img' src='/static/images/arrow-btm.png'></image>
			</view>
		</view>
		<!--只显示一个-->
		<view class='history-data' v-if="ismore==false&&checkRecordFlows.length>0" @click="setMore(true)">
			<view class='content'>
				<view class='content-li' v-for="(item,i) in checkRecordFlows" :key='i' v-if="i<1">
					<view :class='item.opType + " bg"'></view>
					<view class='showBox'>
						<text class='title'>{{item.flowDesc}}</text>
						<text class='time'>{{item.opTime}}</text>
			...
			<view class='right-icon'>
				<image class='img' src='/static/images/arrow-top.png'></image>
			</view>
		</view>
        <view class='dot-contBox'>
            ...
            <view class='subBtn' @click="subDot(details.pointId,details.mapJson)" v-if="roles!=='点位站长'&&isSubmit==true">
                <text class='text'>重新上报</text>
            </view>
			...
			<view class='subBtn' @click="todotReport" v-if="roles=='点位站长'&&isSubmit==true">
			    <text class='text'>点位处理</text>
			</view>
...
<script>
    ...
    import {addAvatarPrefix,isSystem,addAvatarPrefix2} from '@/common/lib/utils.js'
	...
	export default {
		...
		methods: {
			...
			<view class='report-showcont' v-if='item2.itemId!==item.itemId'>
			    ...
			    <view>
			        <text class='title' style='color: #999;'>未检查</text>
			    </view>
			</view>
			*/
		}
