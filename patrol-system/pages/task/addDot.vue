	...
	<view class="content dotBg">
		<view class='dot-top report-top addDotBox'>
		    <view class='report-left'>
		        <view class='imgBox blueBg'>
		            <image :src='setImgs()' class='img'></image>
		        </view>
		    </view>
		...
		<view class='dot-top report-top addDotBox'>
		    ...
		    <view class='report-right'>
		        <input class='addDot-input' v-model="dotName" placeholder="请输入点位名称" />
		    </view>
			<view class='report-icon'>
				<image src='/static/images/arrow.png' class='img'></image>
			</view>
...
<script>
    ...
    import {dotSubmitRandom,getManrandom} from '@/api/user.js'
	...
	export default {
		...
		methods: {
                    ...
                    success: function (res) {
                        ...
                        _that.qqmapsdk.reverseGeocoder({
                            ...
                            success(res){
								...
								_that.dotName= res.result.address
								_that.Street = res.result.address_reference.town.title
                            },
                            fail(err){
                                console.log(err)
                            }
            ...
            subDot(){
				...
				_ars.dimension = (this.latitude).toFixed(6)
				_ars.longitude = (this.longitude).toFixed(6)
...
<view class='dot-top report-top addDotBox' style="margin-top: 32rpx;">
		    <view class='report-left'>
		       <text class='text'>点位长</text>
		    </view>
		    <view class='report-right'>
				<text class='status'>{{roleName}}</text>
				<text class='phone' @click="callPhone(rolePhone)">{{rolePhone}}</text>
		    </view>
			<view class='report-icon' @click="getMan()">
				<image src='/static/images/icon-refresh.png' class='img'></image>
			</view>
		</view>
-->
