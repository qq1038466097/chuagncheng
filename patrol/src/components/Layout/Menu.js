  ...
  const getMenus = (menuTreeN, siderFoldN) => {
    return menuTreeN.map((item) => {
      ...
      if(item.id==777){
        userDatas.funs=null
        userDatas.orgs=null
        userDatas.roles=null
        let _b = srceenUrl+'/screen/html/index.html?datas='+ JSON.stringify(userDatas)
        return (
        ...
        userDatas.orgs=null
        userDatas.roles=null
        let _b = srceenUrl+'/screen2/html/index.html?datas='+ JSON.stringify(userDatas)
        return (
          <Menu.Item key={item.id}>
            <a href={_b} target='_blank'>
              {item.imgUrl && <Icon type={item.imgUrl} />}
