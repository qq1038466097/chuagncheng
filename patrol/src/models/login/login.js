    ...
    effects: {
		* login ({
			...
			if(data.code==200){
				...
				//本地缓存
				//localStorage.setItem('userDataCC', JSON.stringify(data.data.user))
				/*
				...
				//setCookie('userDataCC2',data.data.user,{name:'date',time:1});
				sessionStorage.setItem('userDataCC3', JSON.stringify(data.data.user))
				yield put(routerRedux.push({
					pathname: '/dot',
				}))
				location.reload()
